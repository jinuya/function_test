class Store{
    ...
    MakeStoreItem({id,photo,product_name,brand,price,price_num,num}){
        let dom = document.createElement("div");
        dom.innerHTML = `<div class="card store_card">
                            <img src="resources/images/${photo}" data-id="${id}" alt="store_img" title='store_img' class="card-img" draggable="true">
        ...
        dom.firstChild.querySelector("img").addEventListener("dragstart",this.drag);
    ...
    MakeBasketItem(){
        ...
        this.basketList.forEach((x,idx)=>{
            ...
            dom.innerHTML =`<td>${idx+1}</td>
                            ...
                            <td>${item.brand}</td>
                            <td>${item.price}원</td>
                            <td><input type="number" min="1" data-id="${item.id}" value="${item.num}" class="basket_num form-control w-50"></td>
