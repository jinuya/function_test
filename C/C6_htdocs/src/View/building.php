    ...
    <div class="content_box" id="building_post_list">
        <?php foreach($data[0] as $post):?>
            ...
            <div class="card m-2">
                ...
                <hr>
                <p class="card-text">견적 개수 : <?=$post->num?>개</p>
                <?php if($_SESSION['user']->id == $post->writer_id):?>
                    <button class="m-2 float-right btn btn-dark building_see_id_btn" data-target="#building_see_popup" data-toggle="modal" data-id="<?=$post->id?>">견적보기</button>
                <?php endif;?>
                <?php if($_SESSION['user']->specialist == 1 && $post->status == "requesting" && $flag):?>
                    <button class="m-2 float-right btn btn-primary building_request_id_btn" data-target="#building_request_popup" data-toggle="modal" data-id="<?=$post->id?>">견적 보내기</button>
                <?php endif;?>
                </div>
            </div>
        <?php endforeach;?>
