class App{
    ...
    BasketItemNumber = e =>{
        ...
        if(value < 1){e.target.value = 1;value = 1;}
    ...
    MakeReceipt(){
        ...
        let fontSize = 30,receipt_list = [],item;
    ...
    search = e =>{
        ...
        this.itemlist_area.innerHTML = "";
        if(val !== ""){
            if(this.cho.includes(val[0])){
                for(let i = 0; i < list.length; i++){
                    flag = false;
                    let {id,photo,brand,product_name,price,priceNum,cho_product_name,cho_brand,number} = list[i];
                    // 상품명
